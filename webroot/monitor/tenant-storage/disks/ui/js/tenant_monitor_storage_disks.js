...
cephOSDsView = function() {
    ...
    function populateDisksTree(obj) {
        ...
        $(disksTreeDS).on('change',function() {
            var data = dataSource.getItems();
            if (data.length != 0) {
                if (obj['chartType'] == "Host Tree")
                    updateDisksTreeView(data);
                if (obj['chartType'] == "Bullet Chart")
                    populateStorageBullets(data);
            }
        });
        //if cached data is available trigger event to update
        if(result['lastUpdated'] != null && (result['error'] == null || result['error']['errTxt'] == 'abort')){
    ...
    function onTabActivate(e, ui) {
        ...
        if (selTab == "Scatter Plot") {
            ...
            populateDisksTree({chartType: "Host Tree"});
            ...
            if(tenantStorageChartsInitializationStatus['hostBulletChart']) {
                ...
                populateDisksTree({chartType: "Bullet Chart"});
            }
        }
    }
...
function populateStorageBullets(data) {
    if (data != null)
        data = data[0];
    var hostsBulletInfo = ifNull(data,tenantStorageDisksView.osdsTreeData),
